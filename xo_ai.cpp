...
int main() {
	...
	for (int i = 0; i < 9; ++ i)
		scanf("%lf", inputs + i);
	nn->setInputs(inputs);
	// For testing and saving weights to file
	// double* test_board = new double[9] {0, -1, 0, 1, 1, 0, -1, 1, -1};
	// nn->setInputs(test_board);
	// nn->print();
	// tc->start_training();
	// cout << "Correctness %: " << 100 * tc->verify() << endl;
	// nn->setInputs(test_board);
	// nn->print();
	// nn->save("nn_conf.txt");
	int nn_guess_idx = -1;
	...
	for (int i = 0; i < 9; ++ i) {
		...
		if (inputs[i] == 0.0 && nn_guess_idx == -1) {
			nn_guess_idx = i;
			max_nn_guess_val = predict;
		}
		if (inputs[i] == 0.0 && predict > max_nn_guess_val) {
			nn_guess_idx = i;
			max_nn_guess_val = predict;
		}
	}
